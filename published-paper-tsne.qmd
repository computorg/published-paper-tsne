...
plot2d(X, y, tsne, sorted_idx).show()
plot2d(X, y, sammon, sorted_idx).show()
plot2d(X, y, isomap, sorted_idx).show()
plot2d(X, y, lle, sorted_idx).show()
```
...
def plot2d(X, y, manifold_method, indices=None):
    ...
    if indices is None:
        X_transformed = X_transformed[indices]
    fig = px.scatter(x=X_transformed[:,0], y=X_transformed[:,1],
            color=y,colors=colors, markers=markers)
    fig.update_xaxes(visible=False)
    fig.update_yaxes(visible=False)
    fig.update_layout(legend= {'itemsizing': 'constant'},
                      plot_bgcolor = "rgba(0, 0, 0, 0)")
